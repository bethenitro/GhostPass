...
const GhostPassRevocationManager: React.FC<GhostPassRevocationManagerProps> = ({
  ...
  const handleRevocation = async () => {
    if (!ghostPassToken) {
      setRevocationResult({
        status: 'error',
        message: t('revocationManager.noTokenAvailable'),
    ...
    if (!revocationReason.trim()) {
      setRevocationResult({
        status: 'error',
        message: t('revocationManager.revocationReasonRequired'),
        timestamp: new Date().toISOString()
      });
      return;
    }
    ...
    return (
      <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-yellow-400 font-medium text-sm">{t('revocationManager.noActivePass')}</span>
        </div>
        <p className="text-gray-400 text-xs">
          {t('revocationManager.noTokenAvailable')}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-semibold text-white">{t('revocationManager.title')}</h3>
      </div>

      {/* Current Token Info */}
      <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-600">
        <h4 className="text-sm font-medium text-white mb-3">{t('ghostPass.title')}</h4>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-slate-400">{t('session.sessionId')}:</span>
          ...
          {walletBindingId && (
            <div className="flex justify-between">
              <span className="text-slate-400">{t('ghostPass.bindingId')}:</span>
              <span className="text-white font-mono text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-green-400">{t('ghostPassModesTester.results')}</span>
          </div>
        </div>
      </div>
        ...
        >
          <div>
            <label className="block text-sm font-medium text-white mb-2">
              {t('revocationManager.revocationReason')}
            </label>
            <textarea
              value={revocationReason}
              onChange={(e) => setRevocationReason(e.target.value)}
              placeholder={t('revocationManager.selectReason')}
          ...
          >
            {t('revocationManager.revokePass')}
          </button>
        </motion.div>
      )}
      ...
      {showConfirmation && !revocationResult && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-red-400 font-medium">{t('revocationManager.confirmVaporization')}</span>
          </div>
          
          <div className="space-y-3 text-sm">
            <p className="text-red-200">
              {t('revocationManager.vaporizeConfirmation')}
            </p>
            
            <div className="p-3 bg-black/20 rounded">
              <div className="text-xs text-slate-400 mb-1">{t('revocationManager.revocationReason')}:</div>
            ...
            <div className="bg-yellow-900/20 border border-yellow-500/30 rounded p-3">
              <div className="flex items-center gap-2 mb-2">
                <AlertTriangle className="w-4 h-4 text-yellow-400" />
                <span className="text-yellow-400 font-medium text-xs">{t('revocationManager.revocationReason')}</span>
              </div>
              <ul className="text-yellow-200 text-xs space-y-1">
                <li>• {t('revocationManager.userRequested')}</li>
                <li>• {t('revocationManager.fraudDetected')}</li>
                <li>• {t('revocationManager.policyViolation')}</li>
                <li>• {t('forms.confirmations.cannotUndo')}</li>
              </ul>
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                {isRevoking ? t('revocationManager.revoking') : t('revocationManager.revoke')}
              </button>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="space-y-1">
              <p className="text-red-400 font-medium">{t('revocationManager.revokePass')}</p>
              <p className="text-red-300 text-sm">{t('revocationManager.failedToRevoke')}</p>
            </div>
          </div>
        </motion.div>
      )}
